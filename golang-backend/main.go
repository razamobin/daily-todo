...
func PortfolioLoginHandler(w http.ResponseWriter, r *http.Request) {
    portfolioEmail := "skool@mailinator.com"

    var user User
    err := db.QueryRow("SELECT id, email, timezone, username FROM users WHERE email = ?", portfolioEmail).Scan(&user.ID, &user.Email, &user.Timezone, &user.Username)
    if err != nil {
        if err == sql.ErrNoRows {
            http.Error(w, "Portfolio user not found", http.StatusNotFound)
        } else {
    ...
    log.Printf("Portfolio user ID %d set in session", user.ID)
